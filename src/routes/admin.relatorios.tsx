...
import { getAdminAnalytics, type AdminAnalytics } from "@/lib/supabase-queries";
import {
  ResponsiveContainer,
  LineChart,
  Line,
...
function Relatorios() {
  ...
  function exportCsv() {
    const rows = [
      ...
      ...analytics.clicksByService.map((item) => [
        "cliques_por_servico",
        item.service,
        item.clicks,
      ]),
      ...analytics.linksByState.map((item) => ["links_por_estado", item.state, item.total]),
    ];
    const csv = rows
